...
class LogInfo: Codable {
    ...
    init() {
        self.creator = Creator(name: "miniTools", version: "1.0", comment: nil)
        self.entries = [Entry]()
    }
}
...
class Creator: Codable {
    ...
    init(name:String = "miniTools",version:String = "1.0",comment:String? = nil) {
        self.name = name
...
//        }
    ...
    func append(session:Session){
        ...
        let startedDateTime = iso8601.string(from: startTime)
        // time
